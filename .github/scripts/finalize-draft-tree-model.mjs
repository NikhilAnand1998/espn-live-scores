...
for (const player of players) {
  ...
  const projectionWeight = projectionItems.reduce((sum, item) => sum + item.weight, 0);
  const projectionEnsemble = projectionWeight
    ? projectionItems.reduce((sum, item) => sum + item.value * item.weight, 0) / projectionWeight
    : baseProjection;
  ...
  const projectionModels = [baseProjection, lineupBeatProjection, giqProjection]
  ...
  const suppliedFloor = validNumber(player.giqFloor, { positive: true });
  const suppliedCeiling = validNumber(player.giqCeiling, { positive: true });
  ...
  player.projectionFloor = suppliedFloor !== null && suppliedFloor < projectionEnsemble
    ? suppliedFloor
    : fallbackFloor;
  player.projectionCeiling = suppliedCeiling !== null && suppliedCeiling > projectionEnsemble
    ? suppliedCeiling
    : fallbackCeiling;
...
meta.modelVersion = 'ensemble-rollout-v5';
...
console.log(`Finalized ${players.length} players; ${meta.finalization.skillPlayersValidated} skill-player uncertainty ranges validated; ${estimatedValueRanks} missing value ranks safely estimated.`);
